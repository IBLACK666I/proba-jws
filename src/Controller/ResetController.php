...
class ResetController extends AbstractController
{
    ...
    public function resetPassword(Request $request, string $token, PasswordEncoder $passwordEncoder): JsonResponse
    {
        ...
        $user = $this->documentManager->getRepository(User::class)->findOneBy(['resetToken' => $token]);
        dd($user);
        $endsin = $user->getResetTokenExpiry();
        if ($endsin < new \DateTime()) {
            return new JsonResponse(['message' => 'Invalid or expired token','time'=>new \DateTime()], Response::HTTP_BAD_REQUEST);
        }
